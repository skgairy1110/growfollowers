  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-[#FAFAFA] p-6 sm:p-10 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <span className="text-xs font-mono text-[#B7FF00]">WEEKLY BLUEPRINT</span>
      </div>
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3, 4, 5, 6, 7].map(i => <div key={i} className="h-20 bg-[#121212] rounded-2xl animate-pulse" />)}
        </div>
      ) : (
        <div className="space-y-4">
          {plan.map((item, index) => (
            <div key={index} className="bg-[#121212] border border-neutral-800 p-6 rounded-2xl flex items-center justify-between">
              <div className="flex items-center gap-4">
                <button
                  onClick={() => toggleComplete(index)}
                  className={`w-6 h-6 rounded-lg border flex items-center justify-center transition ${
                    item.completed ? 'bg-[#B7FF00] border-[#B7FF00] text-black font-bold' : 'border-neutral-700 bg-neutral-900'
                  }`}
                >
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs font-mono font-bold text-[#B7FF00]">{item.day}</span>
