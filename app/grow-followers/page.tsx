  ...
  return (
    <div className="min-h-screen bg-background text-foreground p-6 sm:p-10 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <span className="text-xs font-mono text-primary">GUIDED GROWTH FLOW</span>
      </div>
      ...
      {/* Step 1: Goal */}
      {step === 1 && (
        <div className="bg-card border border-neutral-800 rounded-2xl p-8 space-y-6">
          <h2 className="text-xl font-bold">What do you want to grow?</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <button onClick={() => setGoal('Grow Followers')} className={`p-5 rounded-xl border text-left font-semibold transition ${goal === 'Grow Followers' ? 'border-primary bg-neutral-900 text-primary' : 'border-neutral-800 bg-neutral-900/50 text-white'}`}>
              Grow Followers
            </button>
            <button onClick={() => setGoal('Increase Reach')} className={`p-5 rounded-xl border text-left font-semibold transition ${goal === 'Increase Reach' ? 'border-primary bg-neutral-900 text-primary' : 'border-neutral-800 bg-neutral-900/50 text-white'}`}>
              Increase Content Reach
            </button>
          </div>
          <button onClick={() => setStep(2)} className="bg-primary text-white font-semibold px-6 py-3 rounded-xl hover:bg-primary/90 transition flex items-center gap-2">
      ...
      {step === 2 && (
        <div className="bg-card border border-neutral-800 rounded-2xl p-8 space-y-6">
          ...
          <div className="flex items-center justify-between pt-4">
            ...
            <button onClick={() => { setStep(3); handleRunAI(); }} className="bg-primary text-white font-semibold px-6 py-3 rounded-xl hover:bg-primary/90 transition flex items-center gap-2">
      ...
      {step === 3 && processing && (
        <div className="bg-card border border-neutral-800 rounded-2xl p-16 text-center space-y-4">
      ...
      {step === 4 && (
        <div className="space-y-6">
          <div className="bg-card border border-neutral-800 rounded-2xl p-6 flex items-center justify-between">
            <div>
            ...
            <button onClick={() => setStep(1)} className="text-xs text-primary hover:underline">Reconfigure Criteria</button>
          </div>

          <div className="space-y-4">
            {opportunities.map((opp) => (
              <div key={opp.id} className="bg-card border border-neutral-800 p-6 rounded-2xl flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-neutral-400">{opp.category} • <span className="text-primary font-semibold">{opp.relevanceScore}% relevance</span></div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => handleAction(opp.id, 'approved')} className="px-4 py-2 rounded-xl bg-primary hover:bg-primary/90 text-xs text-white font-semibold transition">Approve</button>
                </div>
              </div>
            ))}
            {opportunities.length === 0 && (
              <div className="bg-card border border-neutral-800 rounded-2xl p-12 text-center text-neutral-400">
